...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, subtitle, textAlign = 'left' }) => (
  <div className={`flex flex-col items-${textAlign} text-${textAlign}`} data-aos="fade-down">
    <div className={`flex items-center justify-between gap-x-8 ${textAlign === 'right' ? 'flex-row-reverse' : 'flex-row'}`}>
      <p className={`text-[24px] font-extrabold md:text-[48px] max-w-[50%] ${textAlign === 'right' ? 'text-right' : 'text-left'}`}>{title}</p>
