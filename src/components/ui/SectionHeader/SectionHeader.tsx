...
const SectionHeader: React.FC<SectionHeaderProps> = ({ title, subtitle, textAlign = 'left' }) => (
  <div className={`flex flex-col items-${textAlign} text-${textAlign}`} data-aos="fade-down">
    <div className={`flex items-center justify-between gap-x-4 ${textAlign === 'right' ? 'flex-row-reverse' : 'flex-row'}`}>
      <p className={`text-[24px] font-extrabold w-[70%] md:w-[50%] md:text-[48px] ${textAlign === 'right' ? 'text-right' : 'text-left'}`}>{title}</p>
